...
PBMClaims_NMHTest <- ("./data/output/PBMClaims_NMHTest.csv")
...
PBMClaims_CETest <- read.csv("./data/output/PBMClaims_CETest.csv")

AMMEligibilityTest <- read.csv("./data/output/PBMClaims_AMMEligible.csv")
...
# Declare Date Range for NMH and IPSD range
...
PBMClaims <- read.csv("./data/DataRaw_PBMClaims_2023-05-31.csv") %>%
mutate(ProjectId_IPSD = sample(1000000000:9999999999, nrow(.)))
...
# write.csv(PBMClaims_IPSDTest, "./data/output/PBMClaims_IPSDTest.csv")
```
...
# Step 3: Back to the IPSDTest data
# Filter out cases with "No Valid IPSD"
filtered_table <- PBMClaims_IPSDTest %>%
...
Now we have our confirmation of IPSD and MDDx.
...
joined_table <- joined_table %>%
  rename(NMHTestDate = IPSDate.y)

# Step 4 Rogue: format as date
joined_table <- joined_table %>%
  mutate(IPSDate.x = as.Date(IPSDate.x),
         NMHTestDate = as.Date(NMHTestDate))

# Step 4 Rogue 1: Compare IPSD and NMH dates
joined_table <- joined_table %>%
  mutate(comparison_result_NMH = case_when(
           IPSDate.x < NMHTestDate ~ "IPSD is prior to NMHTest date",
           NMHTestDate < IPSDate.x ~ "IPSD is after test NMHTest date",
           TRUE ~ NA_character_
         ))

# Step 5: Determine the number of days between IPSDate and NMHTestDate as an absolute value
joined_table <- joined_table %>% 
  mutate(days_between_NMH = abs(as.numeric(IPSDate.x - NMHTestDate)))
...
# Do not write this, it is very large 25 million
...
2.  A valid service for Major Depressive Disorder (MDDx)
3.  A Negative Medication History (NMH)
:::
...
PBMClaims_CETest1$IPSDate.x <- as.Date(PBMClaims_CETest1$IPSDate.x)
...
PBMClaims_CETest1$days_between_CEPre <- as.numeric(PBMClaims_CETest1$IPSDate.x - PBMClaims_CETest1$bhhEffectiveDate)
...
# Calculate the number of days between IPSDate and bhhEffectiveDate
PBMClaims_CETest1$days_between_CEPost <- as.numeric(PBMClaims_CETest1$disenrollmentDate - PBMClaims_CETest1$IPSDate.x)
...
# Do not write, very large 7.9 million
...
# do not write, too big 3.9 million
...
# Join the validation matrix back to the VBP ALL APs combined data to add compliance back into the equation
...
# write.csv(Compliance, "./data/output/VBPQR_withValidationMatrix.csv")
```
...
-   Not-Valid ("No Match"): `r ((nrow(VBP_Unduplicated)) - (Validation_Matrix |> filter(Match == "Match") |> nrow()))` of the `r nrow(VBP_Unduplicated)` members on the VBP Quality Roster were not found among eligibile Health Choice pharmacy claims.
...
The matrix that results from combining the validation status and the compliance status of a given member provides the following results. Compliance status is provided by the VBP Quality Roster. The Alliance has not created a model for the assessment of compliance, yet.
...
# Create a 2 x 2 matrix for the chi square test

# transform for Chi Square test
...
ChiSquare_Barchart <- Compliance |> 
  ...
  scale_x_discrete(labels = c("115 days Tx (Compliant)", " Less than 115 days (Non-Compliant)")) +
  theme(
...
The evaluation of the aggregated Alliance Provider VBP Quality Rosters identified 1,691 eligible members. However, only 16% of those (266 members) could be validated by matching the VBP QR record with an eligible claim.
...
The difference between the validation groups was statistically significant. It is also easily visualized. The implications of this data are considerable. The current AMM performance score for the Alliance is 58.43%, or +1.14 percentage points above the mean. When that score is adjusted to only include cases that can be validated by matching with an eligible claim (the light purple), that performance scores rises to 86.5%, or +30.93 percentage points.

This is significant impact on the performance measurement of Alliance Providers. We recommend further investigation into the data and processes used by Health Choice. In the meantime, it is also recommended to research provider tools and innovations to improve the overall performance of the network while we continue to assess data validation. 
...
MemberFollowUpList_TGC <- 
   merge(x = Validation_Matrix,
        y = PBMClaims_AMMEligible,
        by = "MemberID",
        all = TRUE) |>
  drop_na(Match) |> 
  filter(bhhShortname == "TGC")

# write.csv(MemberFollowUpList_TGC,"./data/output/AMM_MemberFollowUpList_TGC.csv")
```
:::
...
MemberFollowUpList |>
  mutate(IPSDate.x = as.Date(IPSDate.x)) |>
  mutate(month = floor_date(IPSDate.x, "month")) |>
...
```
:::
### Record of objects referenced in the workflow
...
for (object_name in objects) {
  ...
  num_observations <- if (is.data.frame(object)) nrow(object) else NA
  num_variables <- if (is.data.frame(object)) ncol(object) else NA
  
  # Check if observations are NA
  if (!is.na(num_observations)) {
    # Create a data frame with object details
    object_info <- data.frame(Object = object_name,
                              Observations = num_observations,
                              Variables = num_variables)
    
    # Add the object details to the list
    object_details[[object_name]] <- object_info
  }
}

# Combine all object details into a single data frame
all_object_details <- do.call(rbind, object_details)

# Format the table using kable
formatted_table <- kable(all_object_details, format = "markdown", align = "c")
