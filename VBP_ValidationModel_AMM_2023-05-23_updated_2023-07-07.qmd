...
joined_table <- joined_table |> 
  select("clientID", "PrimaryId.x", "ProjectId.x", "ProjectId.y", "genericnme", "dtefilled.x", "dtefilled.y")
...
1. Import the "ValueSetListMy2023.csv" file, which contains a list of diagnosis codes.
2. Create a list called "MDDxList" that stores the diagnosis codes from the "ValueSetListMy2023" dataset.
3. Concatenate the diagnosis codes from "ValueSetListMy2023" into a single string, formatted for SQL query.
4. Query the Health Choice data warehouse for eligible MDDx claims within a specified date range. The query retrieves distinct information such as primaryID, MemberID, begDate, PrimaryDiagnosis, Dx1, Dx2, Dx3, ..., Dx12, and MatchFound. The query joins the claims data with the temporary table "#ValueSetListMy2023" using the diagnosis codes to find matches.
5. Save the results of the MDDx claims query as a CSV file named "MDDxClaims_dateslug.csv".
6. Import the "MDDxClaims_20230602.csv" file, which contains the MDDx claims data.
7. Prepare the MDDxClaims data by renaming the "primaryID" column to "PrimaryId".
8. Add a new column called "ProjectId_MDD" with randomly generated 10-digit numbers.
9. Filter out cases with missing "begDate" values.
10. Rename the "begDate" column to "MDDxDate" and select only the relevant columns.
11. Filter the "PBMClaims_NMHTest" table for cases with "New Start" in the "NMHTestResult" column.
12. Rename the "PrimaryId.x" column to "PrimaryId" in the "filtered_table" dataset.
13. Merge the "PBMClaims_NMHTest" and "MDDxClaimsTest" datasets based on the "PrimaryId" column, keeping all rows from "PBMClaims_NMHTest".
14. Calculate the number of days between the "IPSDate" and "MDDxDate" as an absolute value.
15. Determine the "MDDxTestResult" based on the number of days between the "IPSDate" and "MDDxDate". If the number of days is less than 60, the result is "Positive MDDx"; otherwise, it is "No MDDx".
16. Filter out cases with missing "MDDxDate" values.
17. Save the resulting MDDx test claims as a CSV file named "PBMClaims_MDDxTest.csv" (ChatGPT, personal communication, July, 2023).
...
# Add a column "CETestResult" based on the conditions
PBMClaims_CETest1 <- PBMClaims_CETest1 %>%
...
1. Import the original global members roster from an Excel file.
2. Create a working copy of the global members roster and rename the "x" column to "PrimaryId".
3. Merge the "PBMClaims_MDDxTest" table with the global members data using the "PrimaryId" column.
4. Filter the resulting table to keep only rows where the "MDDxTestResult" is "Positive MDDx".
5. Convert the "IPSDate" and "bhhEffectiveDate" columns to Date format.
6. Calculate the number of days between "IPSDate" and "bhhEffectiveDate".
7. Add a new column called "CEPreTest" based on the condition: if the number of days is greater than or equal to 105, the value is "Continuous Prior Enrollment"; otherwise, it is "No Continuous Prior Enrollment".
8. Convert the "IPSDate" and "disenrollmentDate" columns to Date format.
9. Calculate the number of days between "IPSDate" and "disenrollmentDate".
10. Add a new column called "CEPostTest" based on the conditions: if the number of days is greater than or equal to 231 or if the "disenrollmentDate" is NA, the value is "Continuous Post Prescription Enrollment"; otherwise, it is "No Continuous Enrollment".
11. Add a new column called "CETestResult" based on the conditions: if "CEPreTest" is "Continuous Prior Enrollment" and "CEPostTest" is "Continuous Post Prescription Enrollment", the value is "Continuous Enrollment"; otherwise, it is "No Continuous Enrollment".
12. Save the resulting table as a CSV file named "PBMClaims_CETest.csv".
13. Filter the "PBMClaims_CETest" table to keep only rows where "CETestResult" is "Continuous Enrollment".
14. Select specific columns from the filtered table.
15. Filter the resulting table based on the values in the "bhhShortname" column, keeping only rows with values "CPIH", "TGC", "SBH", "CBI", "WYGC", "SHG", "MMHC", or "LCBHC".
16. Save the filtered table as a CSV file named "PBMClaims_AMMEligible.csv" (ChatGPT, personal communication, July, 2023).
...
-   Valid ("Match"): `r Validation_Matrix |> filter(Match == "Match") |> nrow()` of the `r nrow(VBP_Unduplicated)` members on the VBP Quality Roster were also found among eligible Health Choice pharmacy claims. 
-   Not-Valid ("No Match"): `r ((nrow(VBP_Unduplicated)) - (Validation_Matrix |> filter(Match == "Match") |> nrow()))` members on the VBP Quality Roster were not found among eligibile Health Choice pharmacy claims.

#### Compliance

The compliance status of cases on the Health Choice VBP Report were as follows.

-   Non-Compliant: `r Compliance |> filter(Gap_Status == "OPEN") |> nrow()` cases were not found to have the expected duration of antidepressant medication treatment.

-   Compliant: `r Compliance |> filter(Gap_Status == "CLOSED") |> nrow()` cases were found to have the expected duration of antidepressant medication treatment.
