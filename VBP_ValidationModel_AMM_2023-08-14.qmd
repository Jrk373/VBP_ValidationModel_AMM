...
```{r}
# Function to check if two periods are continuous
are_periods_continuous <- function(enrollment1, disenrollment1, enrollment2, disenrollment2) {
  if (any(is.na(c(enrollment1, disenrollment1, enrollment2, disenrollment2)))) {
    return(FALSE)
  }
  return(enrollment2 <= (disenrollment1 + 1))
}

# Create a new data frame to store continuous enrollment periods
continuous_periods <- data.frame(PrimaryID = integer(0), EnrollmentPeriod = character(0))

# Iterate over each PrimaryID
for (id in unique(AllianceEnrolledMembers_Segments$PrimaryID)) {
  id_data <- AllianceEnrolledMembers_Segments %>% filter(PrimaryID == id)
  
  continuous_period <- list()
  
  for (i in 1:(nrow(id_data) - 1)) {
    enrollment1 <- ifelse(is.na(id_data$EnrollmentDate.x[i]), Sys.Date(), id_data$EnrollmentDate.x[i])
    disenrollment1 <- ifelse(is.na(id_data$DisenrollmentDate.x[i]), Sys.Date(), id_data$DisenrollmentDate.x[i])
    enrollment2 <- ifelse(is.na(id_data$EnrollmentDate.x[i + 1]), Sys.Date(), id_data$EnrollmentDate.x[i + 1])
    disenrollment2 <- ifelse(is.na(id_data$DisenrollmentDate.x[i + 1]), Sys.Date(), id_data$DisenrollmentDate.x[i + 1])
    
    if (are_periods_continuous(enrollment1, disenrollment1, enrollment2, disenrollment2)) {
      continuous_period <- c(continuous_period, id_data$EnrollmentDate.x[i], id_data$DisenrollmentDate.x[i + 1])
    } else {
      if (length(continuous_period) > 0) {
        continuous_periods <- bind_rows(continuous_periods, data.frame(PrimaryID = id, EnrollmentPeriod = paste(min(continuous_period), max(continuous_period), sep = " to ")))
      }
      continuous_period <- list()
    }
  }
...
# Resulting data frame with continuous enrollment periods
continuous_periods
