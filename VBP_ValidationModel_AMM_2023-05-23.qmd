...
![NCQA HEDIS My2023 AMM Eligibility Model](./data/DataModel/Graphic_AMMEligibilityModel2023.png)
...
-   Matched (Valid): `r Validation_Matrix |> filter(Match == "Match") |> nrow()` of the `r nrow(VBP_Unduplicated)` VBP members were validated against an eligible AMM Claims member. 
-   Not-Matched (Not Valid): `r ((nrow(VBP_Unduplicated)) - (Validation_Matrix |> filter(Match == "Match") |> nrow()))`
 were not matched.
...
## Impact of the non-Matched member events
...
Some exploratory analysis was conducted on the resulting data from this model. The results are included here for further reference, and we invite any innovations that spring from its review. 
