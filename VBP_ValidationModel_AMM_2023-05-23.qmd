...
[^1]: HEDIS Criteria reference

[^2]: HEDIS Criteria reference

We are working with many of the same principles on this project; however, the AMM guidelines far more nuanced[^3]. Nonetheless we have endeavored to define the eligible population and compare it against the VBP report.

[^3]: HEDIS Criteria reference

The results were great, *(Amazing result to come)*.

We hope you enjoy reading this data story. We are always looking for collaborators to work with, so please reach out and say hello.
...
So, there we were in the first months of the Alliance ACO business, and EVERY provider was concerned that the Value Based Purchasing Quality Roster was inaccurate. Our providers were concerned that their performance scores were underrated.

We want our providers to be be their best selves and be recognized. Lack of confidence in the scoring mechanism crowds out enthusiasm and commitment to improvement.
...
### The VBP Validation Model (2023), key takeaways:

-   Created a validated data model that can be used to identify eligible member events
-   The core of the model can be used to develop future models for other HEDIS/NCQA measures.
-   This research affirmed that the providers were correct. They were indeed underrated for the 2022 measurement year.

:::

Now we arrive to the new measurement year (`r InLineCode$MeasurementYear`), and a new vendor, Cotivity. In this project the VBP Validation Model is going to be applied to the Antidepressant Medication Management (AMM) HEDIS My2023[^4] performance measure. This is the first time that anyone in the HCA network has done this assessment and we hope the results are illuminating.

[^4]: HEDIS Criteria reference

This model compares Report Data from the VBP Quality Report with Real Data from adjudicated claims.
...
Inaccurate measurement of VBP performance leads to an invalid assessment of the Alliance Providers' delivery of services to their patients. It also dampens the spirits of the teams as they do this very difficult work. An accurate measurement offers better opportunity to identify patients in need, maximize financial incentives associated with performance, and support provider morale.
...
[^5]: HEDIS Criteria reference

## Situation Assessment

At the time of this writing, The Alliance has received reports through Cotivity for the first 3 months of the 2023 measurement year. Our application of the VBP Validation Model revealed a similar pattern to 2022 where we found that FUH was underrated. These results for FUH are under further investigation while we also develop this model for AMM.

## Hablemos de dato *(Let's talk about the data)*

This data model is simple in concept, but complex in design. The model compares report data from the HC VBP Quality Roster with real data from HC adjudicated claims.

It is more complicated than it sounds because the eligibility equation for AMM is highly nuanced.

### What are the data sources?

+----------------------------------------------------------------------------------------------------------------------------------+------------+-----------------------------------------------------------------------------------+
| Health Choice VBP Quality Roster (QR)                                                                                            |            | Health Choice Adjudicated Claims                                                  |
+:=================================================================================================================================+============+:==================================================================================+
| -   An excel workbook containing a roster of the members deemed eligible for VBP HEDIS NCQA measures and their compliance status |            | -   A data set of adjudicated claims queried directly from the HCA data warehouse |
|                                                                                                                                  |            |                                                                                   |
...
### Son los datos de calidad? *(Is the data any good?)*

+-------------------------------------------------------------------------------------------------------------------------+------------+--------------------------------------------------------------------------------------------------------------------------------------+
| Health Choice VBP Quality Roster (QR)                                                                                   |            | Health Choice Adjudicated Claims                                                                                                     |
+:========================================================================================================================+============+:=====================================================================================================================================+
| -   Report quality is very high as it is compiled by Health Choice Business Intelligence staff.                         |            | -   Claims data is of the highest quality as it is compiled and reviewed extensively by Health Choice for its own business purposes. |
|                                                                                                                         |            |                                                                                                                                      |
| -   The outstanding question with the VBPQR is if the underlying data from the third party vendor Cotivity is accurate. |            | -   The quality of this data is also reviewed by state and federal regulation entities, like AHCCCS                                  |
+-------------------------------------------------------------------------------------------------------------------------+------------+--------------------------------------------------------------------------------------------------------------------------------------+
...
Check out the visualization of the model below. There are many components of this model with overlapping time frames. We will describe these steps in further detail in the next section.

![AMM Eligibility Model (Kivela, 2023)](./data/DataModel/Graphic_AMMEligibilityModel2023.png)

### National Drug Code to Generic Product Identifier Crosswalk

::: panel-tabset
#### Description {#sec-description}

NDC to GPI Crosswalk
...
-   Import the My2023 Medication to Code data from the My2023 Medication List Directory[^6]
-   Filter the data to only include Antidepressant Medications
-   Rename some columns to provide consistency across tables
-   Import the AHCCCS Preferred Drug List [^7]. A side by side list of NDC and GPI codes is incredibly difficult to come by, and so we are using this list from 2019. Fortunately NDC and GPI are very stable.
-   Rename some columns to provide consistency across tables.
...
[^6]: HEDIS Criteria reference

[^7]: HEDIS Criteria reference

### El Dato Verdad *(The Real Data)*

Now we have to start the arduous process of describing the eligible population. The criteria are complicated. The logic is convoluted. And the syntax is confusing.
...
### The Index Prescription Start Date (IPSD)

::: panel-tabset
#### Description

The Index Prescription Start Date (IPSD) is the anchor point for the rest of the eligibility measures. An individual must have an eligible IPSD in order to be assessed against the other tests for this measure.
...
Now that we have confirmed IPSD, NMH, and MDDx, we will compare the member information against our enrollment records to dtermine there was an eligible enrollment period assoicated with the prescription fill.
...
Take our main table, now called PBMClaims_MDDxTest. Compare the PrimaryId Associated with the event against our member enrollment roster developed in the Alliance Progress Report [^_]. This is the full global members roster because we are not filtering for active membership only. Individuals who are currently disenrolled are included as well.

First, confirm if the eligible event matches anyone on the enrollment roster.

If yes, the compare:
  1.    The IPSD against the enrollment date and confirm that enrollment date is at least 105 days before the IPSD
  2.    The IPSD against the disenrollment date and confirm that the disenrollment date is at least 213 days after the IPSD

[^_]: Alliance progress report (Kivela 2023)
